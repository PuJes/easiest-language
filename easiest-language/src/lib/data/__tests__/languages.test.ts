...
describe('FSI Language Data Validation', () => {
  ...
  describe('Data Structure Integrity', () => {
    test('should have at least 25 languages', () => {
      expect(languages).toHaveLength(28) // We have 28 languages currently
    })
  ...
  describe('FSI Category Validation', () => {
    ...
    test('should have correct distribution of FSI categories', () => {
      const categoryCount = languages.reduce((acc, lang) => {
        const category = lang.fsi.category
        acc[category] = (acc[category] || 0) + 1
        return acc
      }, {} as Record<number, number>)

      // Check that we have languages in each category
      expect(categoryCount[0]).toBeGreaterThanOrEqual(1) // Native (English)
      expect(categoryCount[1]).toBeGreaterThanOrEqual(5) // Category I (easy)
      expect(categoryCount[2]).toBeGreaterThanOrEqual(3) // Category II
      expect(categoryCount[3]).toBeGreaterThanOrEqual(7) // Category III
      expect(categoryCount[4]).toBeGreaterThanOrEqual(2) // Category IV
      expect(categoryCount[5]).toBeGreaterThanOrEqual(3) // Category V (hardest)
    })
  })
  ...
  describe('Difficulty Scores Validation', () => {
    ...
    test('difficulty should generally correlate with FSI category', () => {
      ...
      if (categoryI.length > 0 && categoryV.length > 0) {
        const avgCat1Difficulty = categoryI.reduce((sum, lang) => sum + lang.difficulty.overall, 0) / categoryI.length
        const avgCat5Difficulty = categoryV.reduce((sum, lang) => sum + lang.difficulty.overall, 0) / categoryV.length
